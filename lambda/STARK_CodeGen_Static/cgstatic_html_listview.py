...
def create(data):
    ...
    source_code += f"""\
            <div class="row">
                <div class="col-6">
                    <button type="button" class="btn btn-primary mb-2" onClick="window.location.href='{entity_varname}_add.html'" v-if="{entity}|Add"> <b>+</b> Add </button>
                    <button type="button" class="btn btn-info mb-2" onClick="window.location.href='{entity_varname}_report.html'" v-if="{entity}|Report"> Reports </button>
                </div>
    ...
    source_code += f"""
                        ...
                        <tbody>
                            <template v-for="{entity_varname} in listview_table" id="listview-table">
                                <tr>
                                    <td>
                                        <a :href="'{entity_varname}_edit.html?{pk_varname}=' + {entity_varname}.{pk_varname}"><img src="images/pencil2.png" v-if="{entity}|Edit"></a>
                                    </td>
                                    <th scope="row">
                                        <template id="detail-view" v-if="{entity}|View">
    ...
    source_code += f"""
                                    <td><a :href="'{entity_varname}_delete.html?{pk_varname}=' + {entity_varname}.{pk_varname}"><img src="images/trash2.png" v-if="{entity}|Delete"></a></td>
                                </tr>
                            </template>
                    </tbody>
                    </table>
